...
func TestMeta_Validate(t *testing.T) {
	...
	m.Priority = 1
	errs := m.Validate(registry)
	if errs != nil {
		if len(errs.(*errors.CompositeError).Errors) > 0 {
			t.Fatal("there should be 0 errors, but there were", len(errs.(*errors.CompositeError).Errors), ":", errs.(*errors.CompositeError).Errors)
		}
	}
	...
	errs = m.Validate(registry)
	if errs != nil {
		if len(errs.(*errors.CompositeError).Errors) != 14 {
			t.Fatal("expected 14 errors, but there were", len(errs.(*errors.CompositeError).Errors), ":", errs.(*errors.CompositeError).Errors)
		}
	} else {
		t.Fatal("expected 14 errors, but there were none")
	}
	...
	errs = m.Validate(registry)
	if errs != nil {
		if len(errs.(*errors.CompositeError).Errors) != 3 {
			t.Fatal("expected 3 errors, but there were", len(errs.(*errors.CompositeError).Errors), ":", errs.(*errors.CompositeError).Errors)
		}
	} else {
		t.Fatal("expected 14 errors, but there were none")
	}
