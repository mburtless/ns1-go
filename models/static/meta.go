...
func validate(name string, v reflect.Value, m metaValidation) error {
	var res []error
	...
	if check {
		...
		if !match {
			res = append(res, fmt.Errorf("found type mismatch for meta field '%s'. expected %+v, got: %+v", name, m.kinds, v.Kind()))
		}
	...
	if v.Kind() == reflect.Struct {
		if _, ok := v.Interface().(FeedPtr); !ok {
			res = append(res, fmt.Errorf("if a meta field is a struct, it must be a FeedPtr, got: %s", v.Type()))
		}
	}
...
func (meta *Meta) Validate(formats strfmt.Registry) error {
	var res []error
